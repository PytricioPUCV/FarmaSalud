...
void pause() {
    ...
    getch(); // Espera a que se presione una tecla sin necesidad de Enter
}

void cls() {
    system("cls"); // Cambia a "clear" en Unix-like
}
...
void leerDatosSucursal(struct Sucursal *nuevaSucursal){
    ...
    fflush(stdin); // Limpiar el buffer de entrada después de leer un entero
    nuevaSucursal->nombre = (char *)malloc(50 * sizeof(char));
    printf("Ingrese nombre de la sucursal: ");
    scanf(" %[^\n]", nuevaSucursal->nombre);
    fflush(stdin); // Limpiar el buffer de entrada después de leer un string
...
void leerDatosProveedor(int *id, char **nombre, char **direccion, char **telefono) {
    ...
    scanf("%d", id);
    fflush(stdin);  // Consumir el salto de línea que queda en el buffer
...
void generarDatosDePrueba(struct FarmaSalud *farmacia) {
    ...
    struct Producto *prod1 = crearProductoFalso("1", "Ibuprofeno", "Recomendado para el dolor de garganta", "Anti-inflamatorio", 100, "Bioequivalente", "Lote A", "10/2020", 8, 0);
    struct Producto *prod2 = crearProductoFalso("2", "Condones", "Ultra resistente y ultra delgado", "Anticonceptivo", 200, "Condoneria Nacional", "Lote B", "06/2019", 10, 1);
    struct Producto *prod3 = crearProductoFalso("3", "Paracetamol", "Recomendado para el dolor de cabeza", "Analgesico", 300, "Lab. de Chile", "Lote C", "12/2022", 9, 0); // Producto vencido
    struct Producto *prod4 = crearProductoFalso("4", "Tapsin", "Perfecto Para los resfriados", "Analgésico", 400, "Tapsin Chile", "Lote D", "02/2017", 40, 1);
    struct Producto *prod5 = crearProductoFalso("5", "Amoxicilina", "Antibiótico de amplio espectro", "Antibiótico", 500, "Lab. de Chile", "Lote E", "11/2025", 50, 1);
    struct Producto *prod6 = crearProductoFalso("6", "Loratadina", "Para aliviar la alergia", "Antihistamínico", 600, "PharmaCorp", "Lote F", "10/2024", 60, 0);
    struct Producto *prod7 = crearProductoFalso("7", "Diclofenaco", "Alivia el dolor e inflamación", "Analgesico", 700, "Lab. de Chile", "Lote G", "09/2026", 70, 0);
    struct Producto *prod8 = crearProductoFalso("8", "Aspirina", "Reduce el dolor y la fiebre", "Analgesico", 800, "Bioequivalente", "Lote H", "08/2029", 80, 0);
    struct Producto *prod9 = crearProductoFalso("9", "Omeprazol", "Para el tratamiento de la acidez", "Antiácido", 900, "PharmaCorp", "Lote I", "07/2016", 90, 1);
    struct Producto *prod10 = crearProductoFalso("10", "Vitamina C", "Suplemento vitamínico", "Suplemento", 1000, "Lab. Maver", "Lote J", "06/2026", 100, 0);
